...
class MyClass(Base):
    quote_flag1 = Input(
        widget=Button(generate_quote_fast,
                      label="Quote Fast!"))

    quote_flag2 = Input(
        widget=Button(generate_quote_slow,
                      label="Quote Slow!"))
    ...
    pyfield_bg_color_2 = Input(0, widget=PyField(
    ...
    checkbox_3 = Input("Foo", widget=CheckBox(checked_value="Foo",
                                              unchecked_value="Bar"))
    checkbox_4 = Input(widget=CheckBox(checked_value="Ham",
                                       unchecked_value="Eggs"))

    # no autocompute
    multi_checkbox_1 = Input([True, 'bar'], widget=MultiCheckBox(
        values=options, autocompute=False))
    # no autocompute, use repr instead of str
    multi_checkbox_2 = Input([True, 'bar'], widget=MultiCheckBox(
        values=options, str=repr, autocompute=False))
    # enum
    multi_checkbox_3 = Input([Color.RED, Color.GREEN], widget=MultiCheckBox(
        Color, autocompute=False))
    # autocompute, with labels
    multi_checkbox_4 = Input([True, 'bar'], widget=MultiCheckBox(
        values=options, labels=["True", "2", "bar", "Nothing"]))
    # no autoexpand, custom separator
    multi_checkbox_5 = Input([True, 'bar'], widget=MultiCheckBox(
        values=options, separator=" | ", autoexpand=False))
    ...
    # no autocompute
    single_selection_1 = Input(True, widget=SingleSelection(
        values=options, autocompute=False))
    # no autocompute, use repr instead of str
    single_selection_2 = Input(2.0, widget=SingleSelection(
        values=options, str=repr, autocompute=False))
    # enum
    single_selection_3 = Input(Color.RED, widget=SingleSelection(Color))
    # autocompute, with labels
    single_selection_4 = Input('bar', widget=SingleSelection(
        values=options, labels=["True", "2", "bar", "Nothing"]))
    ...
    single_selection_5 = Input(None, widget=SingleSelection(
        values=options, autoexpand=False))
    # missing required input
    single_selection_6 = Input(widget=SingleSelection(values=options))

    dropdown_1 = Input(widget=Dropdown(options, autocompute=False))
    dropdown_2 = Input(2.0, widget=Dropdown(
        options, str=repr, autocompute=False))
    dropdown_3 = Input(2.0, widget=Dropdown(
        options, labels=["True", "2", "bar", "Nothing"]))
    ...
    filepicker_2 = Input(__file__, widget=FilePicker(
    ...
    # select image dialog
    filepicker_5 = Input(__file__, widget=FilePicker(
    ...
    objectpicker_2 = Input(widget=ObjectPicker(
        multiple=True, ask_confirmation=True))
    # use popup dialog instead of status bar
    objectpicker_3 = Input(widget=ObjectPicker(
        multiple=True, popup=True))
    # validate selection (2 objects)
    objectpicker_4 = Input(derived, widget=ObjectPicker(
        msg="Select 2 objects.", multiple=True,
        validator=objectpicker_validator))
