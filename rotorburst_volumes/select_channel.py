...
class RiskVolume(GeomBase):
    ...
    engine_index = Input(0, widget=Dropdown(number_of_engines, labels=['Left', 'Right'], autocompute=True))

    number_of_stages = list(range(engines[0].shaft.stages.quantify))
    engine_stage_index = Input(0, widget=Dropdown(number_of_stages, labels=['Fan', 'LP-comp', 'HP-comp', 'HP-turb', 'LP-turb']))

    @Part
    def ewis(self):
        return Ewis()

    selected_channel = Input(widget=ObjectPicker(
        multiple=False, ask_confirmation=True))
    @Attribute
    def channel_y(self):
    ...
    def risk_volume_start_point(self):
        return Point(x=self.engine_stage.position[0],
    ...
    def risk_volume(self):
        return Box(width=self.risk_volume_width, length=self.risk_volume_length, height=self.vertex.length*5,
                   centered=False,
                   position=Position((self.risk_volume_start_point)
                                     ).rotate_to(self.risk_volume_start_plane.orientation
                                                 ).rotate('y', self.rotation * pi/180).translate('x', -1* self.risk_volume_width/2),
                   color='red')
