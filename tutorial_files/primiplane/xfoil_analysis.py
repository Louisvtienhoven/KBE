...
class XfoilAnalysis(GeomBase):
    ...
    alpha = Input()  # (alpha_start, alpha_end, step)
    ...
    def section_for_analysis(self):
        return (
            self.section.edges[0].bspline.reversed
            if self.section.edges[0].bspline.plane_normal[1] >= 0
    ...
    def xfoil_analysis(self):  # refer to Xfoil manual for proper use!
        return run_xfoil(
            self.section_points_xfoil,  # run Xfoil analysis
            ...
            cleanup=True,
        )  # remove files generated by xfoil

    plt_backend = Attribute(plt.get_backend())
    ...
    def cl_alpha_plot(self):  # use matplotlib to generate plot
        ...
        plt.title(
            "root: %s - tip: %s  - spanwise position : %s - Flydir?: %s"
            % (
                self.lifting_surface.airfoil_root,
                self.lifting_surface.airfoil_tip,
    ...
    def wing_cutting_plane(self):
        return Plane(
            reference=self.lifting_surface.root_airfoil.location.interpolate(  # point at span percentage
                self.lifting_surface.tip_airfoil.location,
                self.cutting_plane_span_fraction,
            ),
            normal=(
                self.lifting_surface.position.Vy_
                if self.flydir
                else (
                    self.lifting_surface.root_airfoil.location
                    - self.lifting_surface.tip_airfoil.location
                )
            ),
        )  #
    ...
    def section(self):
        return IntersectedShapes(
            shape_in=self.lifting_surface, tool=self.wing_cutting_plane
        )
