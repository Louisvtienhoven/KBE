...
class XfoilAnalysis(GeomBase):
    ...
    def cl_alpha_plot(self):  # use matplotlib to generate plot
        ...
        plt.title("root: %s - tip: %s  - spanwise position : %s - Flydir?: %s" % (self.lifting_surface.airfoil_root,
    ...
    def wing_cutting_plane(self):
        return Plane \
            (reference=self.lifting_surface.root_airfoil.location.  # point at span percentage
             interpolate(self.lifting_surface.tip_airfoil.location, self.cutting_plane_span_fraction),
             normal=
             self.lifting_surface.position.Vy_ if self.flydir
             else
             (self.lifting_surface.root_airfoil.location - self.lifting_surface.tip_airfoil.location))  #
        # leading edge vector
