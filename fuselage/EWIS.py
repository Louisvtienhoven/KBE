...
class Ewis(GeomBase):
    ...
    def upper_channel(self):
        return ChannelX(ch_radius=.1, position=translate(self.position, 'x', 5, 'y', self.channels_ypostion, 'z', self.upper_channel_zposition), color="Blue", ch_length=35.2)

    @Part
    def upper_channel2(self):
        return MirroredShape(shape_in=self.upper_channel,
                             reference_point=self.position,
                             # Two vectors to define the mirror plane
                             vector1=self.position.Vz,
                             vector2=self.position.Vx,
    ...
    def connectorZ2(self):
        return ChannelZ(ch_length=self.upper_channel_zposition - self.lower_channel_zposition,ch_radius=.1, position=translate(self.position, 'x', 7, 'y', 1, 'z', self.lower_channel_zposition), color='Blue')
